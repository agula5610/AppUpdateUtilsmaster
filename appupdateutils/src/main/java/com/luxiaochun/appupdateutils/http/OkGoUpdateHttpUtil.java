...
public class OkGoUpdateHttpUtil implements HttpManager {
     ...
     */
    @Override
    public void download(@NonNull String url, @NonNull String path, @NonNull final Callback callback) {
    ...
    public void remove(@NonNull String url) {
        OkDownload.getInstance().getTask(url).remove();
    }

    class mDownLoadListener extends com.lzy.okserver.download.DownloadListener {
        Callback callback;

        mDownLoadListener(Object tag, Callback callback) {
            super(tag);
